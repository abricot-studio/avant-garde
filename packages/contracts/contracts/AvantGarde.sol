...
contract AvantGarde is ERC721URIStorage {
  ...
  constructor(address _manager, address payable _feesReceiver)
    ERC721('AvantGarde', 'AVG')
  {
    manager = _manager;
  ...
  function mint(string memory _uri, bytes memory _signature)
    public
    payable
  ...
  function mintWithFeesPriceFor(uint256 _current)
    public
    pure
    returns (uint256)
  {
  ...
  function mintPriceFor(uint256 _current)
    public
    pure
    returns (uint256 _currentPrice, uint256 _fees)
  {
