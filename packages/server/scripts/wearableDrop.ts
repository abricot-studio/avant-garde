...
async function getContractCallers(address: string) {
  ...
  const resTxs: any = await axios.get(
    `https://api.etherscan.io/api?module=account&action=txlist&address=${address}&apikey=${config.etherscanKey}`
  )
...
`
  ...
  while (!isEnd) {
    const resOwners = await axios.post(
    ...
    resOwners.data.data.tokens.forEach((entry: any) =>
      poapOwners.add(entry.owner.id.toLowerCase())
    )
