...
import { CircularProgress, CircularProgressLabel, Flex, Text } from './ui'
...
const calculateTimeLeft = (): timeLeft => {
  ...
  const hourProgress = parseInt(
    ((durationLaunch.hours() / 24) * 100).toString()
  )
  const minutesProgress = parseInt(
    ((durationLaunch.minutes() / 60) * 100).toString()
  )
  const secondProgress = parseInt(
    ((durationLaunch.seconds() / 60) * 100).toString()
  )
...
export default function Counter() {
  ...
  return (
    <Flex justifyContent="center" position="relative" flexDirection="column">
      <Text textAlign="center" fontWeight={700} pb={4}>
        Registration ends in
      </Text>
      <Flex>
        <Flex flexDirection="column" mr={4}>
          <CircularProgress
            value={timeLeft.dayProgress}
            size="50px"
            thickness="10px"
            color="reddy"
          >
            <CircularProgressLabel fontWeight={600} color="reddy">
              {timeLeft.dayLaunch}
        ...
        <Flex flexDirection="column" mr={4}>
          <CircularProgress
            value={timeLeft.hourProgress}
            size="50px"
            thickness="10px"
            color="reddy"
          >
            <CircularProgressLabel fontWeight={600} color="reddy">
              {timeLeft.durationLaunch.hours()}
            </CircularProgressLabel>
          </CircularProgress>
          <Text fontSize="0.8rem" textAlign="center">
            Hours
        ...
        <Flex flexDirection="column" mr={4}>
          <CircularProgress
            value={timeLeft.minutesProgress}
            size="50px"
            thickness="10px"
            color="reddy"
          >
            <CircularProgressLabel fontWeight={600} color="reddy">
              {timeLeft.durationLaunch.minutes()}
            </CircularProgressLabel>
          </CircularProgress>
          <Text fontSize="0.8rem" textAlign="center">
            Minutes
        ...
        <Flex flexDirection="column">
          <CircularProgress
            value={timeLeft.secondProgress}
            size="50px"
            thickness="10px"
            color="reddy"
          >
            <CircularProgressLabel fontWeight={600} color="reddy">
              {timeLeft.durationLaunch.seconds()}
            </CircularProgressLabel>
          </CircularProgress>
          <Text fontSize="0.8rem" textAlign="center">
            Seconds
